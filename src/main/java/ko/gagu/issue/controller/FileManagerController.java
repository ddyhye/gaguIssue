...
public class FileManagerController {
	...
	public ResponseEntity<Resource> fileView(@PathVariable String filePath) {
		// 1. 특정 경로에서 파일을 읽어와 Resource 로 만든다
		Resource resource = new FileSystemResource(root + "/" + filePath);
		...
		try {
			if (!Files.exists(Paths.get(root + "/" + filePath))) {
				resource = new ClassPathResource("/static/img/user.jpg");
				header.add("content-type", "image/jpg");
			} else {				
				String type = Files.probeContentType(Paths.get(root + "/" + filePath)); // 경로를 주면 해당 파일의 mime-type 을 알아낸다
	...
	public ResponseEntity<Resource> fileView(@PathVariable String folder, @PathVariable String filePath) {
		...
		// 과정이 있었는데 Spring 에서 이 과정을 대신 해주는 메서드를 제공한다
		try {
			if (Paths.get(root + "/" + folder + "/" + filePath) == null) {
				resource = new ClassPathResource("/static/img/user.jpg");
				header.add("content-type", "image/jpg");
			} else {				
	...
	// [jeong] 클라이언트가 서버에게 파일을 요청시 서버는 해당 경로의 파일을(확장자에 맞춰서) 보내준다.
	@ResponseBody
	@GetMapping("/file/{rootPath}/{subPath}/{filePath}")
	public ResponseEntity<Resource> fileView(@PathVariable String rootPath, @PathVariable String subPath,
			@PathVariable String filePath) {
		// 1. 특정 경로에서 파일을 읽어와 Resource 로 만든다
		Resource resource = new FileSystemResource(root + "/" + rootPath + "/" + subPath + "/" + filePath);
		// 2. 보내질 파일의 형태를 지정해준다
		// 헤더에 보내질 파일의 형태를 지정해준다
		HttpHeaders header = new HttpHeaders();
		...
		// 과정이 있었는데 Spring 에서 이 과정을 대신 해주는 메서드를 제공한다
		try {
			String type = Files.probeContentType(Paths.get(root + "/" + rootPath + "/" + subPath + "/" + filePath)); // 경로를 주면 해당 파일의 mime-type 을 알아낸다
			header.add("content-type", type);
		} catch (IOException e) {
			e.getStackTrace();
		}
